...
const CreateCareer = () => {
  ...
  // Dynamic list helpers (responsibilities & requirements)
  ...
  const handleSubmit = async (e) => {
    ...
    // Clean up empty lines in requirements and responsibilities
    const payload = {
      ...formData,
    ...
    try {
      ...
      setError(
        err.response?.data?.error || err.response?.data?.message || 'Failed to publish job posting to Salesforce.'
      );
      setLoading(false);
    }
  };
      ...
      <div className="bg-white p-8 sm:p-10 rounded-3xl border border-gray-200 shadow-xl space-y-8">
        ...
        <div>
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-pink-100 text-[#2D1B54] text-xs font-bold uppercase tracking-wider mb-2">
            ...
            <span>Publish New Opening</span>
          </div>
          <h1 className="text-2xl sm:text-3xl font-extrabold text-[#2D1B54]">
            Create Career Listing
          </h1>
          <p className="text-xs text-gray-500 mt-1">
            This role will be instantly published to the live public Careers page
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="space-y-1.5">
            ...
            <input
              ...
              placeholder="e.g. Senior Salesforce Technical Architect (Multi-Cloud)"
            ...
            {formData.requirements.map((req, idx) => (
              <div key={idx} className="flex items-center gap-2">
                <input
                  ...
                  placeholder={`Requirement #${idx + 1} (e.g. 5+ years Salesforce Developer experience)`}
                  className="w-full px-4 py-2.5 rounded-xl bg-[#F8F9FD] border border-gray-200 text-xs text-gray-900 focus:bg-white focus:outline-none focus:ring-2 focus:ring-[#6C4AB6]"
                />
          ...
          <div className="pt-6 border-t border-gray-100 flex items-center justify-end gap-3">
            ...
            >
              ...
              <span>{loading ? 'Publishing Role...' : 'Publish Career Posting'}</span>
            </button>
          </div>
