...
const AdminDashboard = () => {
  ...
  const canViewContacts = hasPermission('CONTACT_VIEW');
  const canViewCareers = hasPermission('CAREER_VIEW');
  const canViewApplications = hasPermission('APPLICATION_VIEW');
  const canViewBlogs = hasPermission('BLOG_VIEW');
  const canViewUsers = hasPermission('USER_VIEW');
  const canCreateCareer = hasPermission('CAREER_CREATE');
  const canCreateBlog = hasPermission('BLOG_CREATE');

  const loadDashboardData = async () => {
    setLoading(true);
    try {
      const promises = [];
      if (canViewContacts) promises.push(api.get('/contact')); else promises.push(Promise.resolve({ data: [] }));
      if (canViewCareers) promises.push(api.get('/careers?all=true')); else promises.push(Promise.resolve({ data: [] }));
      if (canViewApplications) promises.push(api.get('/career-submissions')); else promises.push(Promise.resolve({ data: [] }));
      if (canViewBlogs) promises.push(api.get('/blogs')); else promises.push(Promise.resolve({ data: [] }));
      if (canViewUsers) promises.push(api.get('/users')); else promises.push(Promise.resolve({ data: { users: [] } }));

      const [submissionsRes, careersRes, applicationsRes, blogsRes, usersRes] = await Promise.allSettled(promises);

      const subs = (submissionsRes.status === 'fulfilled' && submissionsRes.value?.data) ? submissionsRes.value.data : [];
      const careers = (careersRes.status === 'fulfilled' && careersRes.value?.data) ? careersRes.value.data : [];
      const apps = (applicationsRes.status === 'fulfilled' && applicationsRes.value?.data) ? applicationsRes.value.data : [];
      const blogs = (blogsRes.status === 'fulfilled' && blogsRes.value?.data) ? blogsRes.value.data : [];
      const users = (usersRes.status === 'fulfilled' && usersRes.value?.data?.users) ? usersRes.value.data.users : [];

      setStats({
        totalSubmissions: Array.isArray(subs) ? subs.length : 0,
        newSubmissions: Array.isArray(subs) ? subs.filter(s => s.status === 'new').length : 0,
        totalCareers: Array.isArray(careers) ? careers.length : 0,
        activeCareers: Array.isArray(careers) ? careers.filter(c => c.status === 'active').length : 0,
        totalApplications: Array.isArray(apps) ? apps.length : 0,
        newApplications: Array.isArray(apps) ? apps.filter(a => a.status === 'New').length : 0,
        totalBlogs: Array.isArray(blogs) ? blogs.length : 0,
        totalUsers: Array.isArray(users) ? users.length : 0,
      });

      if (Array.isArray(subs)) setRecentSubmissions(subs.slice(0, 5));
      if (Array.isArray(careers)) setRecentCareers(careers.slice(0, 4));
    } catch (err) {
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="p-6 sm:p-8 rounded-3xl bg-gradient-to-r from-[#2D1B54] via-[#381D66] to-[#4E2F94] text-white shadow-xl flex flex-col md:flex-row md:items-center justify-between gap-6 relative overflow-hidden">
        ...
        <div className="space-y-2 relative z-10">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/10 border border-white/15 text-[11px] font-bold uppercase tracking-wider text-[#FFD1DE]">
            ...
            <span>Array Minds Operations Cockpit</span>
          </div>
          <h1 className="text-2xl sm:text-3xl font-black tracking-tight">
            Welcome back, {admin?.name || 'Administrator'}!
          </h1>
          <p className="text-xs sm:text-sm text-purple-200 font-light">
            Role: <span className="font-bold text-white uppercase bg-white/15 px-2 py-0.5 rounded-md">{admin?.role || 'Staff'}</span>
            {admin?.department && <span className="ml-2">• {admin.department}</span>}
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3 relative z-10">
          ...
          {canCreateBlog && (
            <Link
              ...
              className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-bold shadow-lg shadow-indigo-600/30 transition-all transform hover:-translate-y-0.5"
            >
              <Plus className="w-4 h-4" />
      ...
      {/* ========================================================================= */}
      {/* 2. STATS KPI CARDS (ROLE FILTERED) */}
      {/* ========================================================================= */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        ...
        {canViewBlogs && (
          <Link 
            to="/admin/blogs"
            className="p-6 rounded-3xl bg-white border border-gray-200 shadow-xs hover:shadow-xl hover:border-amber-500/50 transition-all group"
          >
            <div className="flex items-center justify-between">
              <span className="text-xs font-bold text-gray-600 uppercase tracking-wider">
                Blog Publications
            ...
            <div className="mt-4">
              <p className="text-3xl font-black text-[#2D1B54]">{stats.totalBlogs}</p>
              <p className="text-xs text-gray-600 mt-1 flex items-center gap-1">
        ...
        {canViewUsers && (
          <Link 
            to="/admin/users"
            className="p-6 rounded-3xl bg-white border border-gray-200 shadow-xs hover:shadow-xl hover:border-blue-500/50 transition-all group"
          >
            <div className="flex items-center justify-between">
              <span className="text-xs font-bold text-gray-600 uppercase tracking-wider">
            ...
            <div className="mt-4">
              <p className="text-3xl font-black text-[#2D1B54]">{stats.totalUsers}</p>
              <p className="text-xs text-gray-600 mt-1 flex items-center gap-1">
      ...
      {/* ========================================================================= */}
      {(canViewContacts || canViewCareers) && (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          
          {/* Left Column: Recent Inquiries */}
          {canViewContacts && (
            <div className={`${canViewCareers ? 'lg:col-span-7' : 'lg:col-span-12'} bg-white p-6 sm:p-8 rounded-3xl border border-gray-200 shadow-sm space-y-6`}>
          ...
          {canViewCareers && (
            <div className={`${canViewContacts ? 'lg:col-span-5' : 'lg:col-span-12'} bg-white p-6 sm:p-8 rounded-3xl border border-gray-200 shadow-sm space-y-6`}>
              <div className="flex items-center justify-between border-b border-gray-100 pb-4">
                <div>
